...
class Example(Frame):
    def __init__(self, parent):
        ...
        self.OS = platform.system().lower()#判断os是linux,os,windows转换成小写
        self.parent = parent
        self.fileName = ""
        self.debug = False
        ...
        self.controlCommand = {'q':"unTag", 'ctrl+z':'undo'} #ctr+z 操作撤回，q移除带注释的标签
        self.labelEntryList = [] #标签条目表
        self.shortcutLabelList = []#快捷标签表
        self.configListLabel = None #配置列表标签
        self.configListBox = None #配置列表框

        # 默认GUI显示参数
        if len(self.pressCommand) > 20:
            self.textRow = len(self.pressCommand) #行textRow的行默认大于等于20
        else:
            self.textRow = 20
        self.textColumn = 5 #textRow的列默认
        self.tagScheme = "BMES" #标签计划
        self.onlyNP = False  # 出口序列
        self.keepRecommend = True #继续推荐


        '''
        self.seged:
        对于导出序列，如果为真，则用空格分隔单词，否则用空格分隔字符
        例如，如果您的数据被中文(或英文)用空格分隔的单词分隔，则需要将此标志设置为true
        如果你的数据是中文，没有分割，你需要设置这个标志为假
        '''
        self.seged = True  ## 导出序列是非分段汉语时为假，导出序列是分段的英语或汉语时为真
        self.configFile = "configs/default.config"#配置文件
        self.entityRe = r'\[\@.*?\#.*?\*\](?!\#)'#实体正则匹配[],@...,#...,*,
        self.insideNestEntityRe = r'\[\@\[\@(?!\[\@).*?\#.*?\*\]\#'#内嵌实体匹配[@[@...[@
        self.recommendRe = r'\[\$.*?\#.*?\*\](?!\#)'#建议重新匹配
        self.goldAndrecomRe = r'\[\@.*?\#.*?\*\](?!\#)'#
        if self.keepRecommend:
            self.goldAndrecomRe = r'\[[\@\$)].*?\#.*?\*\](?!\#)'
        ## configure color
        self.entityColor = "SkyBlue1"#实体颜色
        self.insideNestEntityColor = "light slate blue"#内嵌实体颜色
        self.recommendColor = 'lightgreen'#机器推荐的颜色
        self.selectColor = 'light salmon'#选择颜色
        self.textFontStyle = "Times"#文本字体样式
        self.initUI()

    #初始用户界面
    def initUI(self):

        self.parent.title(self.Version)#parent是Frame的参数，标题
        self.pack(fill=BOTH, expand=True)#fill = BOTH是将标签填充满

        for idx in range(0,self.textColumn):
            self.columnconfigure(idx, weight =2)
        # self.columnconfigure(0, weight=2)设置行尺寸
        self.columnconfigure(self.textColumn+2, weight=1)
        self.columnconfigure(self.textColumn+4, weight=1)
        for idx in range(0,16):
            self.rowconfigure(idx, weight =1)#设置列尺寸
